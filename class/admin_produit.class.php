...
class admin_produit extends page_secur {
	...
	private function liste_produits() {
		...
		<br />
		<table>
		<tr><th>Libellé</th><th>Description</th><th>Catégorie</th></tr>
		<?php
		// Parcours des r�sultats obtenus
		while ($uneOccurrence = $lesFruits->fetch_object()) {
			?>
			<tr>
			<td><strong><?php echo $uneOccurrence->nom; ?></strong></td>
			<td><?php echo substr($uneOccurrence->description,0,20); ?> ...</td>
			<td><em><?php echo $uneOccurrence->lib; ?></em></td>
			<td><div class="bouton"><a href="?action=modif&id='.$uneOccurrence->id.'">Modifier</a></div></td>
			<td><div class="bouton"><a href="?action=suppr&id='.$uneOccurrence->id.'">Supprimer</a></div></td>
			</tr>
			<?php
		}
		?>
