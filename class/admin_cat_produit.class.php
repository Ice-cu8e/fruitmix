...
class admin_cat_produit extends page_secur {
	private $connect;
	...
	private function form_modif() {
		...
		$requete = 'select * from catproduit where id='.$_GET['id'];
		...
		if (!isset($_GET['action']))
			...
			switch ($_GET['action']) {
				...
				case 'valid_ajout' : // Insertion dans la BdD des informations saisies dans le formulaire
					...
					// exemple : insert into catproduit(lib, texte) values ("Fruits secs", "Lorem Ipsum");
					$requete = 'insert into catproduit(lib, texte)
				...
				case 'valid_modif' : // Modification dans la BdD � partir des informations saisies
					...
					// exemple : update catproduit set lib="aurantiac�es", texte="Lorrem Ipsum" where id=3;
					$requete = 'update catproduit
				...
				case 'valid_suppr' : // Suppression de la cat�gorie et des produits associ�s
					if ($_POST['valid'] == 'valider') { // Si l'utilisateur a valid� la suppression
						...
						if ($result) {
							...
							// exemple : delete from catproduit where id=6;
							$requete = 'delete from catproduit
										where id='.$_POST['id'].';';
							// Ex�cution de la requ�te
							$result = $this->connect->query($requete);
							if ($result)
